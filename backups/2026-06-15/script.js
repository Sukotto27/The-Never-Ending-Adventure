...
// 12.3 · World Interactions
			...
			async function runTutorialIntro() {
				...
				await tutorialHint(
					...
					'<strong>Quest Panel</strong><br>This panel shows your active quest and current objective.',
				);
				...
				await tutorialHint(
					'#wheel-area',
					'<strong>Action Wheel</strong><br>This is your primary way to interact with the world. Every action available to you — exploring, resting, hunting, crafting — starts here.<br><br>It\'s getting late. You should make camp for the night. Before you can set up camp, you\'ll need to look around for a good spot. To search your current area, click <strong>Exploration.</strong>',
				);

				// 7. Time & weather dial
				await tutorialHint(
					'#hud-datetime',
					'<strong>Time & Weather</strong><br>This dial tracks the time of day and current weather. Both affect what actions are available and how the world responds to you.',
				);
			}
			...
			function _tutCheckExploration() {
				...
				setTimeout(async () => {
					await tutorialHint(
						'#wheel-area',
						'<strong>Find Camping Spot</strong><br>Now, select <strong>Find Camping Spot</strong> to complete your first objective.',
			...
			function _tutMakeCampHint() {
				...
				setTimeout(async () => {
					await tutorialHint(
						'#wheel-area',
						'<strong>Make Camp</strong><br>You\'ve found a good spot! Now select <strong>Back</strong> to exit the Exploration wheel, then choose <strong>Survival</strong>',
					);
					await tutorialHint(
						'#hud-player-info',
						'Some actions cost stamina. Your stamina level can be found here, along with your life and mana levels.',
					);
				}, 120);
			}
			...
			function _tutCheckSurvivalForMakeCamp() {
				...
				setTimeout(async () => {
					await tutorialHint(
						'#wheel-area',
						'<strong>Make Camp</strong><br>Now that you\'ve found a good spot, you\'re ready to set up camp. Click <strong>Make Camp</strong>',
			...
			function _tutCheckCampfireForCook() {
				...
				setTimeout(async () => {
					await tutorialHint('#wheel-area', '<strong>Cook</strong><br>Fire is burning! Click <strong>Cook</strong> to cook your raw meat. If your fire goes out, you\'ll have to light it again. Hint: You can prevent this by proactively adding sticks or logs to the fire as it burns.', { pulseLabel: 'Cook' });
				}, 120);
			}
			...
			function _tutCheckShelterWheel() {
				...
				if (!player.hasShelter && !player.flags?.tutBuildShelterShown) {
					...
					setTimeout(async () => {
						await tutorialHint('#wheel-area', '<strong>Sleep</strong><br>Shelter built! Now it\'s time to rest and recover some strength. Click <strong>Sleep</strong> to rest for the night.', { pulseLabel: 'Sleep' });
					}, 120);
				}
			}
			...
			function _tutCheckActionsForWater() {
				...
				setTimeout(async () => {
					await tutorialHint('#wheel-area',
						'<strong>Survival</strong><br>Great hunt! Now you should find a water source and fill up your waterskin. Hydration is important! Click <strong>Survival</strong> to open the survival options.',
			...
			function _tutCheckGatherForFire() {
				...
				setTimeout(async () => {
					await tutorialHint('#wheel-area',
						'<strong>Gather Materials</strong><br>To build and light a campfire you need:<br>• <strong>Gather Stones</strong> — you\'ll need 8 stones.<br>• <strong>Gather Sticks</strong> — you\'ll need at least 1 stick bundle.<br><br>Lastly, you\'ll need kindling. Luckily, you already found some while setting up your camp!');
					await tutorialHint('#wheel-area',
			...
			async function _onTutorialObjectiveAdvance(newIndex) {
				...
				if (newIndex === 1) {
					if (!player.flags?.tutHuntHintShown) {
						...
						await tutorialHint(
							'[data-bksec="inventory"]',
							'<strong>Equip Your Hunting Bow</strong><br>Your campsite is ready! There\'s a little time left before nightfall. You should use this time to hunt for food. Before you can hunt, you must have a bow equipped!. Click the <em>Inventory</em> tab to view your items.',
							{ advance: 'click' }
						);
						await new Promise(r => setTimeout(r, 150));
						const bowName = Object.keys(player.inventory).find(k => /bow/i.test(k) && (player.inventory[k].quantity ?? 1) > 0) || 'Hunting Bow';
						...
						await tutorialHint(
							'#wheel-area',
							'<strong>Hunt</strong><br>You\'re back at the Survival wheel. With your bow equipped, you\'re ready to hunt. Click <strong>Hunt</strong> to begin.',
							{ pulseLabel: 'Hunt' }
						);
					}
				...
				// ── Objective 4: cook_food ────────────────────────────────────────────
					...
					if (!player.flags?.tutCookHintShown) {
						...
						await tutorialHint(
							'#wheel-area',
							'<strong>Cook Your Food</strong><br>Fire is burning! Click <strong>Cook</strong> and select your raw meat. If your fire goes out, you\'ll have to light it again. Hint: You can prevent this by proactively adding extra sticks or logs to the fire.',
						);
					}
				...
				// ── Objective 5: eat_food ─────────────────────────────────────────────
					...
					if (!player.flags?.tutEatHintShown) {
						...
						await tutorialHint(
							'[data-bksec="inventory"]',
							'<strong>Eat the Food</strong><br>Open your <em>Inventory</em> tab to eat the food you cooked.',
							{ advance: 'click' }
						);
						await new Promise(r => setTimeout(r, 150));
				...
				// (Index 8 = talk_bandits — handled automatically by _tutorialBanditEncounter)
					...
					if (!player.flags?.tutVillageHintShown) {
						...
						await tutorialHint(
							'[data-bksec="map"]',
							'<strong>Find a Village</strong><br>You\'ll need some gold to get by in this world. You should find a village and look for work. Click the <em>Map</em> tab to plan your route.',
							{ advance: 'click' }
						);
				...
				// ── Objective 10: find_work ───────────────────────────────────────────
					...
					if (!player.flags?.tutWorkHintShown) {
						...
						await tutorialHint(
							'#wheel-area',
							'<strong>Find Work</strong><br>You\'ve reached a settlement. Choose <strong>Town</strong> to access local activities — look for <em>Odd Jobs</em> or speak with villagers to find paid work and earn back some coin.',
						);
					}
