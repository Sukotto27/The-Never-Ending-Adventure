...
const SoundManager = (() => {
  ...
  const SOUNDS = {
    // Actions
    chop_wood:                'sounds/actions/chop_wood.mp3',
    ...
    // Exploration / quests  (placeholder — files not yet created)
    item_pickup:              'sounds/effects/item_pickup.mp3',
    quest_update:             'sounds/effects/quest_update.mp3',
    quest_complete:           'sounds/effects/quest_complete.mp3',
    equip:                    'sounds/effects/equip.mp3',
  ...
  function updateAmbience() {
    ...
    if (/city|capitalcity|village/.test(zone)) {
      setAmbience(isNight ? 'ambience_town_night' : 'ambience_town_day');
      return;
    }

    if (hasFire) { setAmbience('ambience_campfire'); return; }
