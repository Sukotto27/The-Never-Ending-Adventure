...
function _convShowCategoryWheel(npc) {
  ...
  // If the player hasn't learned this person's name yet, show a visual description instead
  const _isUnknown = npc._isStranger && rel.score <= 0 && !rel.conversationLog?.length;
  if (_isUnknown) {
    const _base = (npc.appearance || '').split('.')[0].trim().toLowerCase();
    const _r    = npc.race || 'Human';
    const _vis  = (_r === 'Human') ? _base : `${_r.toLowerCase()} — ${_base}`;
    _addStory(`<strong>Conversing with a stranger</strong> &mdash; <em>${_vis}</em>`);
  } else {
    _addStory(`<strong>Talking to ${npc.name}</strong> &mdash; <em>${relLabel}</em>`);
  }
  _buildWheelC(options, _isUnknown ? 'A Stranger' : npc.name);
}
